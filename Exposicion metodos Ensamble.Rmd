...
##    4.1   MODELO - Arbol de Decision (Decision Tree Classifier)
...
###   4.5.1    Generaccion del MODELO - C5.0
```{r}
library(C50)
...
## MaBoost (Mirror Ascent Boosting) (AdaBoost - Multiclase)

"Maboost" se utiliza para adaptarse a una variedad de modelos de impulso estocásticos para respuestas binarias y multiclase
como se describe en A Boosting Framework on Grounds of Online Learning por T. Naghibi y B. Pfister,
(2014).


```{r}
library(maboost)
mod_mab  <- maboost ( training_set2 [, - 1 ], training_set2 [, 1 ], iter  =  200 , nu  =  .02 )
```

```{r}
predMAB  <- predict( mod_mab , test_set2 [, - 1 ])
```


```{r}
print(data.frame(test_set2, predMAB))
```

```{r}
confMatMb <- confusionMatrix(test_set2[,1], predMAB)
print(confMatMb)
```
